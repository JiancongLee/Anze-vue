  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/sys/user/deptlist'),
          method: 'get',
          params: this.$http.adornParams({
            'page': this.pageIndex,
            'limit': this.pageSize,
            'username': this.dataForm.userName,
            'deptList': this.deptList.join(',')
          })
        }).then(({data}) => {
          if (data && data.code === 0) {
      ...
      handleNodeClick (data, node, self) {
        if (node.checked) {
          this.$refs.dept_tree.setChecked(node, false, true)
        } else {
          this.$refs.dept_tree.setChecked(node, true, true)
        }
      },
      handleNodeCheck (data, node, self) {
        if (node) {
          this.deptList.push(data.id)
          this.getDataList()
        } else {
          this.deptList = this.deptList.filter(item => item !== data.id)
          this.getDataList()
        }
      },
