...
<script>
  import AddOrUpdate from './shopsearchhistory-add-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    mounted () {
      this.offsetHeight = this.$refs.shopsearchhistoryFrom.offsetHeight
    },
    methods: {
      // 获取数据列表
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/shopsearchhistory/list'),
          method: 'get',
      ...
      toggleSelection (row) {
        if (row) {
          this.$refs.shopsearchhistoryTable.toggleRowSelection(row)
        } else {
          this.$refs.shopsearchhistoryTable.clearSelection()
        }
      },
      // 新增 / 修改
      ...
      exportExcel () {
        ...
        this.$axios({
          url: this.$http.adornUrl(`/shopsearchhistory/export?token=${this.$cookie.get('token')}`),
          method: 'GET',
          responseType: 'blob',
          params: {
            id: '',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/shopsearchhistory/delete'),
            method: 'post',
