...
<script>
  import AddOrUpdate from './shopbrand-add-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    mounted () {
      this.offsetHeight = this.$refs.shopbrandFrom.offsetHeight
    },
    methods: {
      // 获取数据列表
      getDataList () {
      ...
      toggleSelection (row) {
        if (row) {
          this.$refs.shopbrandTable.toggleRowSelection(row)
        } else {
          this.$refs.shopbrandTable.clearSelection()
        }
      },
      // 新增 / 修改
      ...
      exportExcel () {
        ...
        this.$axios({
          url: this.$http.adornUrl(`/shopbrand/export?token=${this.$cookie.get('token')}`),
          method: 'GET',
          responseType: 'blob',
          params: {
            id: '',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/shopbrand/delete'),
            method: 'post',
