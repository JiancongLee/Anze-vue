  ...
  <el-container>
    ...
    <el-container class="center-container" direction="vertical">
      <el-header class="btn-bar" style="height: 45px;">
        ...
        <el-button v-if="this.ifSelect" type="text" size="medium" icon="el-icon-printer" @click="handleLoad">加载表数据</el-button>
        <el-button type="text" size="medium" icon="el-icon-view" @click="handlePreview">预览</el-button>
        <el-button type="text" size="medium" icon="el-icon-tickets" @click="handleGenerateJson">生成JSON</el-button>
        <el-button type="text" size="medium" icon="el-icon-document" @click="generateCodeServer">生成代码</el-button>
...
export default {
  name: 'fm-making-form',
  props: ['dbinfoId', 'tableName', 'ifSelect'],
  components: {
    Draggable,
    WidgetConfig,
    FormConfig,
  ...
  mounted () {
    ...
    loadJs('static/plugins/ace/src/ace.js')
  },
  methods: {
    handleGoGithub () {
    ...
    handleLoad () {
      this.$http.get(this.$http.adornUrl('/code/getItems'), {params: this.$http.adornParams({dbinfoId: this.dbinfoId, tableName: this.tableName})})
        .then(({data}) => {
          if (data && data.code === 0) {
            let _this = this
            _this.widgetForm.list = []
            this.widgetForm.config.formName = data.item.name + 'Form'
            this.widgetForm.config.chinaName = data.item.chinaName
            data.item.fields.forEach(function (value) {
              const key = Date.parse(new Date()) + '_' + Math.ceil(Math.random() * 99999)
              let fieldName = value.chinaName || '单行文本'
              let input = {
                type: 'input',
                name: fieldName,
                model: _this.widgetForm.config.formName + '.' + value.name,
                key: key,
                ...
                options: {
                  ...
                  remoteFunc: 'func_' + value.fieldName
                },
                rules: []
              }
              console.log(value)
              _this.widgetForm.list.push(input)
            })
            if (this.widgetForm.list.length) {
              this.widgetFormSelect = _this.widgetForm.list[0]
            }
          }
        })
    },
    ...
    generateCodeServer () {
      ...
      this.$http({
        ...
        method: 'post',
        responseType: 'blob',
        data: this.$http.adornData(this.widgetForm)
      }).then((response) => {
